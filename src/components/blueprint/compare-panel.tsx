...
export function ComparePanel({ history, current }: Props) {
  ...
  useEffect(() => {
    if (leftId) return;
    const fallback = current?.id ?? history[0]?.id ?? "";
    if (fallback) setLeftId(fallback);
  }, [current, history, leftId]);
  ...
  return (
    <div className="panel p-4 sm:p-5 space-y-3">
      ...
      <label className="block space-y-1">
        ...
        <select
          className="flex h-11 w-full rounded-[var(--radius-sm)] border border-border bg-input px-2 text-base text-fg"
          value={leftId}
