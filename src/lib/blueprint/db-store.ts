...
export async function saveBlueprintDb(bp: Blueprint): Promise<void> {
  ...
  `;
    ...
    /* COSY blueprints table is a different shape — scanner history stays local. */
  }
}
...
export async function clearBlueprintsDb(
  ownerId: string | null,
  ...
  if (!ownerId) {
    return {
      ok: false,
      error:
        "Clear all is unavailable without a signed-in owner. Sign in or delete items individually.",
    };
  }
  const sql = await getSql();
  await sql`delete from blueprints where owner_id = ${ownerId}`;
  return { ok: true };
}
