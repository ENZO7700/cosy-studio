...
export function compileElementorTemplate(opts: {
  ...
  notes.push(
    `Compiled ${stats.nodes} nodes (${stats.widgets} widgets). Draft import: Templates → Saved Templates → Import — re-bind JetEngine/dynamic fields after import.`,
  );
  notes.push(
    "Best-effort DOM compile — not a drop-in Elementor template; JetEngine listings and dynamic fields need re-binding after import.",
  );
