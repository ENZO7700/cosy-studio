...
import { SCANNER_USER_AGENT } from "@/lib/scanner/user-agent";
...
export async function scanToBlueprint(input: ScanRequest): Promise<Blueprint> {
  ...
  const wantAssets = input.captureAssets === true;
  ...
  if (wantWp && source !== "html") {
    try {
      ...
      if (wordpress.detected) {
        notes.push(
          `WP/JetEngine public extract: WP=${wordpress.isWordPress} Jet=${wordpress.isJetEngine} Elementor=${wordpress.isElementor}; CCT=${wordpress.cctTypes.length}; listings=${wordpress.listingGrids.length}.`,
        );
      }
    } catch (err) {
      notes.push(
  ...
  // Elementor DOM → draft / best-effort template JSON (not drop-in)
  try {
    const tpl = compileElementorFromBlueprint(blueprint);
    blueprint.elementorTemplate = tpl;
    notes.push(
      `Elementor draft compiled (best-effort DOM): ${tpl._blueprint?.widgetCount ?? 0} widgets, ${tpl.content.length} top containers — re-bind JetEngine/dynamic fields after import.`,
    );
  } catch (err) {
    notes.push(
