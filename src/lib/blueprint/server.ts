...
import { checkScanRateLimit } from "./rate-limit";
...
function scanRateLimitKey(): string {
  try {
...
export const scanBlueprint = createServerFn({ method: "POST" })
  ...
  .handler(async (ctx) => {
    const data = ctx.data;
    const rate = checkScanRateLimit(scanRateLimitKey());
    if (!rate.allowed) {
      return {
        ok: false as const,
        error: `Scan rate limit exceeded. Try again in ${rate.retryAfterSec ?? 60}s.`,
        code: "RATE_LIMIT",
      };
    }
    const signal =
...
export const clearBlueprints = createServerFn({ method: "POST" }).handler(
  async () => {
    ...
    try {
      const result = await clearBlueprintsDb(user?.id ?? null);
      if (!result.ok) {
